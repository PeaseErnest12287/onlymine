...
// Fun Messages and Emojis for the tag command
const funMessages = [
...
Asena.addCommand(
  ...
  async (message, match) => {
    ...
    if (match == "all") {
      let funMessage = funMessages[Math.floor(Math.random() * funMessages.length)];
      let mesaj = `${funMessage}\n\n`;

      for (let i = 0; i < mentionedJid.length; i++) {
        let userName = await getName(mentionedJid[i], message.client);
        let funEmoji = funEmojis[Math.floor(Math.random() * funEmojis.length)];
        mesaj += `${i + 1}. @${mentionedJid[i].split("@")[0]} - ${userName} ${funEmoji}\n`;
      }
    ...
    forwardOrBroadCast(message.jid, message, {
      contextInfo: { mentionedJid },
    });
  }
);
